 ...
 * @date 01/08/19
 */
public class DFS_Dijkstra_Bellman_1 {
    ...
    public DFS_Dijkstra_Bellman_1(int vertexCount) {
    ...
    public static void main(String[] args) throws Exception {
        DFS_Dijkstra_Bellman_1 graph = new DFS_Dijkstra_Bellman_1(4);
        graph.addVertex('A'); //0
        graph.addVertex('B'); //1
        graph.addVertex('C'); //2
        graph.addVertex('D'); //3
    ...
    public void bellmanFord(Integer[][] matrix, int source) throws Exception {
        ...
        /*
        Try relaxing one more time, if relaxation happens, then there exists a negative cycle
         */
        ...
        printResult(minDist, source);
    }

    private void printResult(int[] minDist, int source) {
        System.out.println("Distances from::" + vertices[source].lable + " are::");
        for (int i = 0; i < vertexCount; i++) {
            System.out.print(vertices[source].lable + " to " + vertices[i].lable + " are:: " + minDist[i]);
            System.out.println();
        }
    }
